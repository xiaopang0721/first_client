		...
		*/
		__proto.changeValue = function () {
			if (this.sizeGrid) {
				...
				var end = left + right + sw;
				if (this.isTween && end > this._bar.width) {
					this.isTweenEnd = false;
					Laya.Tween.clearTween(this._bar)
					Laya.Tween.to(this._bar, { width: end }, 200, null, laya.utils.Handler.create(this, function () {
		...
		*/
		__proto.changeCellState = function (cell, visable, index) {
			...
			if (selectBox) {
				...
				selectBox.visible = selectBox.clipY == 3 ? true : visable;
				selectBox.index = visable ? (selectBox.clipY == 3 ? index + 1 : index) : 0;


				// selectBox.visible=selectBox.clipY==2?visable:true;
				// selectBox.index=visable?(selectBox.clipY==3 && index == 1) ? index + 1:index:0;
			}
		}
