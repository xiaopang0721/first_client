...
(function (gamecomponent) {
    ...
    (function (scene) {
        var MapLayer = /** @class */ (function (_super) {
            ...
            function MapLayer(txe, v) {
                var _this = _super.call(this, v.game) || this;
                _this._changeSize = true;
                _this._game = v.game;
                _this._scene = v;
                _this._viewPort = new Rectangle();
                _this._bottomLayer = new Sprite();
                _this._bottomLayer.mouseEnabled = false;
                _this.addChild(_this._bottomLayer);
                return _this;
            }
            MapLayer.prototype.load = function (info) {
                var _this = this;
                if (this._mapTexture) {
                ...
                var url = StringU.substitute("{0}pz_{1}.png", CompoentPath.map, imgid);
            ...
            MapLayer.prototype.onLoadComplete = function (txe) {
                if (!txe) {
                    return;
                }
            ...
            //设置窗口大小
            MapLayer.prototype.resize = function (w, h, realW, realH) {
            ...
            MapLayer.prototype.update = function (diff) {
                if (!this._mapTexture) {
                    return;
                }
                if (!this._changeSize)
                ...
                var state = 0; // 0：铺满视口，1：铺满全屏, 2：单图拉伸
                if (state == 0) {
            ...
            MapLayer.prototype.onDraw = function (diff) {
                var bG = this._bottomLayer.graphics;
                //清理画布
                bG.clear();
                // this._avatarDjBg && this._avatarDjBg.onDraw(diff, bG, this._scene);
            };
            MapLayer.MAP_LOAD_COMPLETE = 'MAP_LOAD_COMPLETE';
            return MapLayer;
        }(game.base.Container));
