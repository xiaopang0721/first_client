    ...
    (function (managers) {
        var VipMgr = /** @class */ (function (_super) {
            ...
            function VipMgr(game) {
                ...
                _this._vipLevel = WebConfig.info.vip_level;
            ...
            VipMgr.prototype.onVipUpdate = function (type) {
                if (type == 1) { //vip等级变更
                    ...
                    if (!this._game.sceneGame.sceneObjectMgr.mapInfo && this._vipLevel != this._game.sceneGame.sceneObjectMgr.mainPlayer.playerInfo.vip_level) {
                        this._game.uiRoot.general.open(DatingPageDef.PAGE_VIP_UP);
                        this._vipLevel = this._game.sceneGame.sceneObjectMgr.mainPlayer.playerInfo.vip_level;
                    }
                }
            ...
            VipMgr.prototype.checkVipReceived = function () {
                if (!WebConfig.info)
                    return false;
                //vip是否有未领取奖励，有的话返回true
                for (var i = 1; i <= WebConfig.info.vip_level; i++) {
                    if (WebConfig.info.vip_received.indexOf(i) == -1) {
                        return true;
                    }
                }
                return false;
