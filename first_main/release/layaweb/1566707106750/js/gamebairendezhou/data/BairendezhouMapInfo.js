var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
/**
* 地图信息
*/
var gamebairendezhou;
(function (gamebairendezhou) {
    var data;
    (function (data) {
        var BairendezhouMapInfo = /** @class */ (function (_super) {
            __extends(BairendezhouMapInfo, _super);
            function BairendezhouMapInfo(v) {
                var _this = _super.call(this, v, function () { return new data.BairendezhouData(); }) || this;
                _this.areaName = ["蓝", "红", "和", "一对", "两对", "三条/顺子", "同花/葫芦", "金刚/同花顺"];
                _this.cardType = ["高牌", "一对", "两对", "三条", "顺子", "同花", "葫芦", "金刚", "同花顺", "皇家同花顺"];
                return _this;
            }
            //当对象更新发生时
            BairendezhouMapInfo.prototype.onUpdate = function (flags, mask, strmask) {
                _super.prototype.onUpdate.call(this, flags, mask, strmask);
                var isNew = flags & core.obj.OBJ_OPT_NEW;
                if (isNew || mask.GetBit(MapField.MAP_INT_MAP_BYTE)) {
                    this._sceneObjectMgr.event(BairendezhouMapInfo.EVENT_STATUS_CHECK);
                }
                if (isNew || mask.GetBit(MapField.MAP_INT_BATTLE_INDEX)) {
                    this._battleInfoMgr.OnUpdate();
                    this._sceneObjectMgr.event(BairendezhouMapInfo.EVENT_BATTLE_CHECK);
                }
                if (isNew || mask.GetBit(MapField.MAP_INT_COUNT_DOWN)) {
                    this._sceneObjectMgr.event(BairendezhouMapInfo.EVENT_COUNT_DOWN);
                }
                if (isNew || strmask.GetBit(MapField.MAP_STR_GAME_NO)) {
                    this._sceneObjectMgr.event(BairendezhouMapInfo.EVENT_GAME_NO);
                }
                if (isNew || strmask.GetBit(MapField.MAP_STR_GAME_RECORD)) {
                    this._sceneObjectMgr.event(BairendezhouMapInfo.EVENT_GAME_RECORD);
                }
                if (isNew || strmask.GetBit(MapField.MAP_STR_SEATED_LIST)) {
                    this._sceneObjectMgr.event(BairendezhouMapInfo.EVENT_SEATED_LIST);
                }
                if (isNew || strmask.GetBit(MapField.MAP_STR_ROAD_RECORD)) {
                    this._sceneObjectMgr.event(BairendezhouMapInfo.EVENT_ROAD_RECORD);
                }
                if (isNew || strmask.GetBit(MapField.MAP_STR_CARD_RECORD)) {
                    this._sceneObjectMgr.event(BairendezhouMapInfo.EVENT_CARD_RECORD);
                }
            };
            BairendezhouMapInfo.prototype.getBattleInfoToString = function () {
                var playerArr = this._battleInfoMgr.users;
                if (!playerArr)
                    return "";
                var selfSeat = -1;
                for (var i = 0; i < playerArr.length; i++) {
                    var player = playerArr[i];
                    if (player && this._sceneObjectMgr.mainPlayer.GetAccount() == player.account) {
                        //找到自己了
                        selfSeat = i + 1;
                        break;
                    }
                }
                if (selfSeat == -1)
                    return "";
                var infoArr = this._battleInfoMgr.info;
                if (!infoArr)
                    return "";
                var totalStr = "";
                var betArr = [0, 0, 0, 0, 0, 0, 0, 0];
                var betStr = [];
                var lotteryStr = "";
                var awardStr = "";
                var settleStr = "";
                for (var i = 0; i < infoArr.length; i++) {
                    var info = infoArr[i];
                    if (info.SeatIndex == selfSeat) {
                        //自己的战斗日志
                        if (info instanceof gamecomponent.object.BattleInfoAreaBet) {
                            //下注信息
                            betArr[info.BetIndex - 1] += info.BetVal;
                        }
                        else if (info instanceof gamecomponent.object.BattleInfoSettle) {
                            //结算
                            settleStr = info.SettleVal == 0 ? "0" : info.SettleVal > 0 ? "+" + EnumToString.getPointBackNum(info.SettleVal, 2) : "" + EnumToString.getPointBackNum(info.SettleVal, 2);
                            break;
                        }
                    }
                    else if (info instanceof gamecomponent.object.BattleLogCardsResult) {
                        for (var j = 0; j < info.Results.length; j++) {
                            if (j < this.areaName.length) {
                                //中奖区域
                                if (info.Results[j] == 0) {
                                    if (!awardStr) {
                                        awardStr = this.areaName[j];
                                    }
                                    else {
                                        awardStr += ", " + this.areaName[j];
                                    }
                                }
                            }
                            else {
                                //开奖详情
                                if (!lotteryStr) {
                                    lotteryStr = "蓝:" + this.cardType[info.Results[j]];
                                }
                                else {
                                    lotteryStr += ",  " + "红:" + this.cardType[info.Results[j]];
                                }
                            }
                        }
                    }
                }
                var count = 0;
                var index = 0;
                for (var i = 0; i < betArr.length; i++) {
                    if (betArr[i] > 0) {
                        if (count == 3) { //汉字太多了3个一行
                            count = 0;
                            index++;
                        }
                        if (!betStr[index]) {
                            betStr[index] = StringU.substitute("{0}({1})", this.areaName[i], betArr[i]);
                        }
                        else {
                            betStr[index] += " , " + StringU.substitute("{0}({1})", this.areaName[i], betArr[i]);
                        }
                        count++;
                    }
                }
                //开奖信息
                totalStr += "开　　奖：|" + lotteryStr + "#";
                //中奖信息
                totalStr += "中　　奖：|" + awardStr + "#";
                //下注信息
                for (var i = 0; i < betStr.length; i++) {
                    if (i == 0) {
                        totalStr += "玩家下注：|" + betStr[i] + "#";
                    }
                    else {
                        totalStr += "|" + betStr[i] + "#";
                    }
                }
                //结算信息
                totalStr += "玩家盈利：|" + settleStr;
                return totalStr;
            };
            //地图状态变更
            BairendezhouMapInfo.EVENT_STATUS_CHECK = "BairendezhouMapInfo.EVENT_STATUS_CHECK";
            //战斗体更新
            BairendezhouMapInfo.EVENT_BATTLE_CHECK = "BairendezhouMapInfo.EVENT_BATTLE_CHECK";
            //牌局号
            BairendezhouMapInfo.EVENT_GAME_NO = "BairendezhouMapInfo.EVENT_GAME_NO";
            //倒计时时间戳更新
            BairendezhouMapInfo.EVENT_COUNT_DOWN = "BairendezhouMapInfo.EVENT_COUNT_DOWN";
            //游戏记录更新
            BairendezhouMapInfo.EVENT_GAME_RECORD = "BairendezhouMapInfo.EVENT_GAME_RECORD";
            //入座列表更新
            BairendezhouMapInfo.EVENT_SEATED_LIST = "BairendezhouMapInfo.EVENT_SEATED_LIST";
            //大路记录变化
            BairendezhouMapInfo.EVENT_ROAD_RECORD = "BairendezhouMapInfo.EVENT_ROAD_RECORD";
            //牌型记录变化
            BairendezhouMapInfo.EVENT_CARD_RECORD = "BairendezhouMapInfo.EVENT_CARD_RECORD";
            return BairendezhouMapInfo;
        }(gamecomponent.object.MapInfoT));
        data.BairendezhouMapInfo = BairendezhouMapInfo;
    })(data = gamebairendezhou.data || (gamebairendezhou.data = {}));
})(gamebairendezhou || (gamebairendezhou = {}));
//# sourceMappingURL=BairendezhouMapInfo.js.map