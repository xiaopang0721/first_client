...
(function (gamesangong) {
    ...
    (function (page_1) {
        ...
        var SangongMapPage = /** @class */ (function (_super) {
            __extends(SangongMapPage, _super);
            function SangongMapPage(v, onOpenFunc, onCloseFunc) {
            ...
            // 页面初始化函数
            SangongMapPage.prototype.init = function () {
                ...
                if (!this._SangongMgr) {
                    this._SangongStory = this._game.sceneObjectMgr.story;
            ...
            // 页面打开时执行函数
            SangongMapPage.prototype.onOpen = function () {
                var _this = this;
            ...
            //打开时要处理的东西
            SangongMapPage.prototype.updateViewUI = function () {
            ...
            //按钮点击
            SangongMapPage.prototype.onBtnTweenEnd = function (e, target) {
                switch (target) {
                    ...
                    case this._viewUI.btn_cardtype:
                        this._game.uiRoot.general.open(page_1.SangongPageDef.PAGE_SG_RULE, function (page) {
                            page.dataSource = 1;
                        });
                        break;
                    case this._viewUI.btn_rules:
                        this._game.uiRoot.general.open(page_1.SangongPageDef.PAGE_SG_RULE);
                        break;
                    ...
                    case this._viewUI.btn_record:
                        this._game.uiRoot.general.open(TongyongPageDef.PAGE_TONGYONG_RECORD, function (page) {
                            page.dataSource = page_1.SangongPageDef.GAME_NAME;
                        });
                        break;
            ...
            //点击任意地方关闭菜单
            SangongMapPage.prototype.onMouseClick = function (e) {
            ...
            //选择下注倍数
            SangongMapPage.prototype.onBet = function (index, e) {
                if (e.currentTarget) {
            ...
            SangongMapPage.prototype.onUnitAdd = function (u) {
                this.onUpdateUnit();
            };
            //玩家出去了
            SangongMapPage.prototype.onUnitRemove = function (u) {
                this.onUpdateUnit();
                var posIdx = (u.GetIndex() - this._mainIdx + 5) % 5;
                this._viewUI["view_banker" + posIdx].visible = false;
            };
            //精灵显示
            SangongMapPage.prototype.onUpdateUnit = function (qifu_index) {
                var _this = this;
                var mapinfo = this._game.sceneObjectMgr.mapInfo;
                if (!mapinfo)
                    return;
                ...
                var _loop_1 = function (index) {
                    var posIdx = (idx + index) % maxCount == 0 ? 5 : (idx + index) % maxCount;
                    ...
                    if (unit) {
                        ...
                        if (this_1._curStatus != 8 /* MAP_STATE_SETTLE */ || this_1._SangongMgr.isReLogin) {
                            this_1.updateMoney();
                        }
                        //头像框
            ...
            SangongMapPage.prototype.playTween = function (img, index, isTween) {
                if (!img)
                    return;
            ...
            SangongMapPage.prototype.onOptHandler = function (optcode, msg) {
                var _this = this;
            ...
            //金币变化
            SangongMapPage.prototype.updateMoney = function () {
            ...
            //地图监听
            SangongMapPage.prototype.onUpdateMapInfo = function () {
                var _this = this;
                var mapInfo = this._game.sceneObjectMgr.mapInfo;
            ...
            //假精灵数据
            SangongMapPage.prototype.onUpdateUnitOffline = function () {
                if (!this._SangongMgr.unitOffline)
                    return;
                var unitOffline = this._SangongMgr.unitOffline;
            ...
            //地图状态
            SangongMapPage.prototype.onUpdateMapState = function () {
                if (!this._mapInfo)
                    return;
                var mainUnit = this._game.sceneObjectMgr.mainUnit;
                if (!mainUnit)
                ...
                if (state >= 6 /* MAP_STATE_BET */) {
                    this._viewUI.img_qiang.visible = false;
                    for (var i = 1; i < 5; i++) {
                        this._viewUI["view_player" + i].img_banker.visible = false;
                    }
                    for (var i = 0; i < 5; i++) {
                        var unitIdx = (idx + i) % 5 == 0 ? 5 : (idx + i) % 5;
                ...
                if (state == 6 /* MAP_STATE_BET */) {
                    ...
                    for (var i = 0; i < this._bankerTemp.length; i++) {
                        ...
                        var index = (this._bankerTemp[i] - mainUnit.GetIndex() + 5) % 5;
                        if (unit.GetIdentity() == 1) {
                    ...
                    var bankePer = bankerMoney / (8 * ChipConfig[this._SangongStory.mapLv][0]);
                    var selfMoney = mainUnit.GetMoney();
                    var xianPer = selfMoney / (2 * ChipConfig[this._SangongStory.mapLv][0]);
                    var betPer = bankePer > xianPer ? xianPer : bankePer;
                    if (betPer > 35) {
                        betPer = 35;
                    }
                    for (var i = 1; i < 6; i++) {
                        var val = Math.floor(i * betPer / 5 + 0.5);
                ...
                if (state == 7 /* MAP_STATE_SHOW_CARDS */) {
                ...
                if (state == 9 /* MAP_STATE_END */) {
                    this._pageHandle.pushClose({ id: page_1.SangongPageDef.PAGE_SG_WIN, parent: this._game.uiRoot.HUD });
                    this._pageHandle.pushClose({ id: page_1.SangongPageDef.PAGE_SG_LOSE, parent: this._game.uiRoot.HUD });
                    this._viewUI.btn_continue.visible = true;
                }
                this._pageHandle.updatePageHandle(); //更新额外界面的开关状态
                this._pageHandle.reset(); //清空额外界面存储数组
            };
            SangongMapPage.prototype.randBanker = function () {
            ...
            //发完牌了
            SangongMapPage.prototype.onAfterDealCards = function () {
                this._viewUI.view_fapai.visible = false;
                this._viewUI.view_fapai.ani1.stop();
            };
            //更新倒计时时间戳
            SangongMapPage.prototype.updateCountDown = function () {
                var mapinfo = this._game.sceneObjectMgr.mapInfo;
                if (!mapinfo)
                    return;
            ...
            //操作倒计时
            SangongMapPage.prototype.deltaUpdate = function () {
                if (!(this._game.sceneObjectMgr.mapInfo instanceof SangongMapInfo))
                    return;
                if (!this._viewUI)
                    return;
                if (this._curStatus != 4 /* MAP_STATE_BANKER */ && this._curStatus != 6 /* MAP_STATE_BET */) {
                    this._viewUI.img_time.visible = false;
            ...
            //战斗日志
            SangongMapPage.prototype.updateBattledInfo = function () {
                var mainUnit = this._game.sceneObjectMgr.mainUnit;
                if (!mainUnit)
                    return;
                    ...
                    switch (battleInfo.Type) {
                        ...
                        case 3: { //摊牌
                            if (this._battleIndex < i) {
                                ...
                                var posIdx = (idx - mainIdx + 5) % 5;
                                var cardType = this._SangongMgr.checkCardsType(cards);
                                var type = void 0;
                                var per = void 0; //倍数
                                if (cardType > 1) {
                                    type = cardType + 8;
                                ...
                                if (!this._SangongMgr.isReLogin) {
                                    ...
                                    if (unit) {
                                        ...
                                        var sexType = headNum > 10 ? "female" : "male";
            ...
            //庄家赢钱
            SangongMapPage.prototype.addBankerWinEff = function () {
                var _this = this;
                ...
                Laya.timer.once(timeInternal, this, function () {
                    ...
                    if (_this._moneyChange >= 0) {
                        _this._game.uiRoot.general.open(page_1.SangongPageDef.PAGE_SG_WIN);
                        ...
                        _this._game.uiRoot.general.open(page_1.SangongPageDef.PAGE_SG_LOSE);
                ...
                if (this._settleLoseInfo.length < 1)
                    return;
                var mainIdx = mainUnit.GetIndex();
                var bankerPos = (this._bankerIdx - mainIdx + 5) % 5;
                for (var i = 0; i < this._settleLoseInfo.length / 2; i++) {
                    var index = i * 2;
                    var unitPos = (this._settleLoseInfo[index] - mainIdx + 5) % 5;
            ...
            //庄家输钱
            SangongMapPage.prototype.addBankerLoseEff = function () {
                //通吃
                ...
                for (var i = 0; i < this._settleWinInfo.length / 2; i++) {
                    var index = i * 2;
                    var unitPos = (this._settleWinInfo[index] - mainIdx + 5) % 5;
            ...
            //金币变化 飘金币特效
            SangongMapPage.prototype.addMoneyFly = function (fromPos, tarPos) {
            ...
            //金币变化 飘字clip
            SangongMapPage.prototype.addMoneyClip = function (value, pos) {
                var idx = this._game.sceneObjectMgr.mainUnit.GetIndex();
                var valueClip = value >= 0 ? new SangongClip(SangongClip.ADD_MONEY_FONT) : new SangongClip(SangongClip.SUB_MONEY_FONT);
            ...
            //清理飘钱动画
            SangongMapPage.prototype.clearClip = function () {
            ...
            //洗牌动画之后
            SangongMapPage.prototype.afterXiPai = function () {
                var _this = this;
            ...
            //充值弹框
            SangongMapPage.prototype.onNotEnoughMoney = function () {
                var _this = this;
                if (!this._game.sceneObjectMgr.mainPlayer)
                    return;
                if (this._game.sceneObjectMgr.mainPlayer.playerInfo.money < ChipConfig[this._SangongStory.mapLv][1]) {
                    TongyongPageDef.ins.alertRecharge(StringU.substitute("老板，您的金币少于{0}哦~\n补充点金币去大杀四方吧~", ChipConfig[this._SangongStory.mapLv][1]), function () {
            ...
            //重置数据
            SangongMapPage.prototype.resetData = function () {
                this._battleIndex = -1;
            ...
            SangongMapPage.prototype.clearMapInfoListen = function () {
            ...
            SangongMapPage.prototype.close = function () {
                if (this._viewUI) {
            ...
            return SangongMapPage;
        }(game.gui.base.Page));
        page_1.SangongMapPage = SangongMapPage;
    })(page = gamesangong.page || (gamesangong.page = {}));
})(gamesangong || (gamesangong = {}));
//# sourceMappingURL=SangongMapPage.js.map